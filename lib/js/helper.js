...
const ns = { // ns is the shortcut for namespace
  element(selector) {
    const s = ns.selectable(this)
    return s.querySelector(selector)
  },

  elements(selector) {
    return ns.selectable(this).querySelectorAll(selector)
  },
  ...
  rect() {
    const b = ns.tag(this).getBoundingClientRect()
  ...
  async elementOverlay(id, msg) {
    const interval = 100
    const el = ns.tag(this)

    let pre = el.getBoundingClientRect()
    await ns.overlay(id, pre.left, pre.top, pre.width, pre.height, msg)
  ...
  retry(fn, next = requestAnimationFrame) {
    const ctrl = new AbortController()
    const wait = new Promise((resolve, reject) => {
      const job = async () => {
        try {
          const ret = await fn(ctrl.signal)
          if (!ctrl.signal.aborted && ret === null) {
            next(job)
          } else {
            resolve(ret)
          }
